...
func findMedianSortedArrays(nums1 []int, nums2 []int) float64 {
	...
	var left = (n1 + n2 + 1) / 2
	var right = (n1 + n2 + 2) / 2
	res := float64(getKth(nums1, nums2, 0, n1, 0, n2, left) + getKth(nums1, nums2, 0, n1, 0, n2, right))
	return res / 2
}
...
func getKth(nums1 []int, nums2 []int, n1I int, n1J int, n2I int, n2J int, k int) int {
	...
	locationK1 := n1I + minInt(k/2, len1) - 1
	locationK2 := n2I + minInt(k/2, len2) - 1
	if nums1[locationK1] > nums2[locationK2] {
		// 序列2更小
		return getKth(nums1, nums2, n1I, n1J, locationK2+1, n2J, k-minInt(k/2, len2))
	} else {
		// 是locationK1 + 1 因为当前值也不要了 处理的是“从locationK1之后开始的序列”
		return getKth(nums1, nums2, locationK1+1, n1J, n2I, n2J, k-minInt(k/2, len1))
	}
}
